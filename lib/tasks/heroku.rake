HEROKU_CONFIG_FILE = Padrino.root('config', 'heroku.yml')
...
namespace :heroku do
  ...
  task :create do
    each_heroku_app do |name, app, repo|
      sh "heroku create #{app} --stack bamboo-ree-1.8.7"
    end
  end
  ...
  task :gems => 'gems:base' do
    ...
    gems = Padrino.configuration.gems.reject { |g| g.frozen? && !g.framework_gem? }
    list = gems.collect do |g|
      command, *options = g.send(:install_command)
      options.join(" ")
    end

    list.unshift(%Q{padrino --version "= #{Padrino.version}"})

    File.open(Padrino.root('.gems'), 'w') do |f|
      f.write(list.join("\n"))
    end
  end
  ...
  desc 'Lists apps configs'
  task :listconfig => :all do
    each_heroku_app do |name, app, repo|
      sh "heroku config --long --app #{app}"
    end
  end
  ...
  desc 'Add PADRINO_LOG_LEVEL=test to each application'
  task :log_level => :all do
    each_heroku_app do |name, app, repo|
      command = "heroku config:add PADRINO_LOG_LEVEL=test --app #{app}"
      puts command
    end
  end
...
#    Rake::Task[:after_deploy].execute - FIXME
  end
  ...
  namespace :db do
    task :pull do
      each_heroku_app do |name, app, repo|
        ...
        sh "mkdir -p #{Padrino.root}/db/dumps"
        file = "#{Padrino.root}/db/dumps/#{dump}.sql.gz"
        url = `heroku pgdumps:url --app #{app} #{dump}`.chomp
        sh "wget", url, "-O", file
        sh "rake db:drop db:create"
        sh "gunzip -c #{file} | #{Padrino.root}/script/dbconsole"
        sh "rake jobs:clear"
      end
    end
  end
  ...
  def migrate(app)
    sh "heroku rake --app #{app} ar:migrate"
    sh "heroku restart --app #{app}"
  end
